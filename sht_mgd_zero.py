...
class Search:
    ...
    def find_valid_id(self):
        for id in self.ids:
            if id and id != -1:
                maximum = max(abs(self.shot.data[id][1]))
                if maximum > self.noise_val:
                    self.valid_id = id
                    return
...
def main():
    ...
    for n in numbers:
        shot = Shot(number=n,
                    shtpath="./sht",
                    unpack_method="exe")
        #search = Search(shot, list(map(str, sys.argv[3].split(","))), sys.argv[4], float(sys.argv[5]))
        search = [Search(shot=shot,
                        names=["МГД  зонд T2","continuum","test","МГД быстрый зонд T2"],
                        cond='<',
                        cond_val=1.2,
                        noise_val= 0.3,
                         time= [0.0,0.150]),
                  Search(shot=shot,
                         names=["Ip новый (Пр1ВК) (инт.16)", "Ip внутр.(Пр2ВК) (инт.18)", "Ip+Ivv нар.(Пр1ВК) (инт.16)", "Ip внутр.(Пр2ВК) (инт.18)"],
                         cond='>',
                         cond_val=0.3,
                         noise_val=0.3,
                         time=[0,0.200])
